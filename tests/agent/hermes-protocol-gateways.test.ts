...
describe('Hermes protocol gateway readiness', () => {
  it('reports MCP, A2A, ACP, and bridge capabilities without claiming editor packaging parity', () => {
    ...
    expect(rendered).toContain('Notes: The stdio ACP transport supports initialize, session/new, in-process session/load replay, session/prompt, session/cancel, and agent-to-client JSON-RPC request/response correlation.');
  });
