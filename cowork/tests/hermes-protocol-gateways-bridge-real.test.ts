...
const hasBuiltProtocolCore = fs.existsSync(
  path.join(distRoot, 'agent', 'hermes-protocol-gateways.js')
);
...
describe.skipIf(!hasBuiltProtocolCore)(
  'Hermes protocol gateways bridge real core integration',
  () => {
    let originalEnv: Partial<Record<EnvKey, string | undefined>>;

    beforeEach(() => {
      originalEnv = Object.fromEntries(envKeys.map((key) => [key, process.env[key]])) as Partial<
