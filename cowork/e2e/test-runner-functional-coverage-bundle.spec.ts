...
test('runs the safe functional coverage bundle from the test runner window', async ({
  appPage,
}) => {
  ...
  await expect(bundleRow).toContainText('functional coverage bundle');
  await expect(bundleRow).toContainText('chat');
  ...
  await appPage.screenshot({
    path: path.resolve(
      process.cwd(),
      '../docs/qa/code-buddy-studio/screenshots/63-test-runner-functional-coverage-bundle.png'
    ),
    fullPage: true,
  });
});
