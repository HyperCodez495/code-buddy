...
test('runs the A2A ACP channel bundle from the test runner window', async ({ appPage }) => {
  ...
  const bundleId = 'code-buddy-a2a-acp-channel-bundle';
  const bundleRow = appPage.getByTestId(`test-catalog-row-${bundleId}`);
  await expect(bundleRow).toBeVisible();
  await expect(bundleRow).toContainText('A2A / ACP channel bundle');
  await expect(bundleRow).toContainText('remote routing');
  await expect(bundleRow).toContainText('channel bridge');
  ...
  await appPage.screenshot({
    path: path.resolve(
      process.cwd(),
      '../docs/qa/code-buddy-studio/screenshots/69-test-runner-a2a-acp-channel-bundle.png'
    ),
    fullPage: true,
  });
});
