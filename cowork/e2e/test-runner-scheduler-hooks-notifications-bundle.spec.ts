...
test('runs the Scheduler hooks notifications bundle from the test runner window', async ({
  appPage,
}) => {
  ...
  const bundleId = 'code-buddy-scheduler-hooks-notifications-bundle';
  const bundleRow = appPage.getByTestId(`test-catalog-row-${bundleId}`);
  await expect(bundleRow).toBeVisible();
  await expect(bundleRow).toContainText('scheduler hooks notifications bundle');
  await expect(bundleRow).toContainText('cron prechecks');
  await expect(bundleRow).toContainText('webhooks');
  ...
  await appPage.screenshot({
    path: path.resolve(
      process.cwd(),
      '../docs/qa/code-buddy-studio/screenshots/88-test-runner-scheduler-hooks-notifications-bundle.png'
    ),
    fullPage: true,
  });
});
