...
test('runs the CLI command surface bundle from the test runner window', async ({ appPage }) => {
  ...
  const bundleId = 'code-buddy-cli-command-surface-bundle';
  const bundleRow = appPage.getByTestId(`test-catalog-row-${bundleId}`);
  await expect(bundleRow).toBeVisible();
  await expect(bundleRow).toContainText('command surface bundle');
  await expect(bundleRow).toContainText('headless exit codes');
  await expect(bundleRow).toContainText('run recall');
  ...
  await appPage.screenshot({
    path: path.resolve(
      process.cwd(),
      '../docs/qa/code-buddy-studio/screenshots/79-test-runner-cli-command-surface-bundle.png'
    ),
    fullPage: true,
  });
});
