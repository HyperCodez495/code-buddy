...
test('runs the Cowork remote control bundle from the test runner window', async ({ appPage }) => {
  ...
  const bundleId = 'code-buddy-cowork-remote-control-bundle';
  const bundleRow = appPage.getByTestId(`test-catalog-row-${bundleId}`);
  await expect(bundleRow).toBeVisible();
  await expect(bundleRow).toContainText('remote control bundle');
  await expect(bundleRow).toContainText('Remote manager');
  await expect(bundleRow).toContainText('slash-command remote');
  ...
  await appPage.screenshot({
    path: path.resolve(
      process.cwd(),
      '../docs/qa/code-buddy-studio/screenshots/94-test-runner-remote-control-bundle.png'
    ),
    fullPage: true,
  });
});
