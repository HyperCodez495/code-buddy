...
test('runs the Data session sync cache bundle from the test runner window', async ({ appPage }) => {
  ...
  const bundleId = 'code-buddy-data-session-sync-cache-bundle';
  const bundleRow = appPage.getByTestId(`test-catalog-row-${bundleId}`);
  await expect(bundleRow).toBeVisible();
  await expect(bundleRow).toContainText('session sync cache bundle');
  await expect(bundleRow).toContainText('Database layer');
  await expect(bundleRow).toContainText('distributed cache');
  ...
  await appPage.screenshot({
    path: path.resolve(
      process.cwd(),
      '../docs/qa/code-buddy-studio/screenshots/83-test-runner-data-session-sync-cache-bundle.png'
    ),
    fullPage: true,
  });
});
