...
test('runs the Agent reasoning execution bundle from the test runner window', async ({ appPage }) => {
  ...
  const bundleId = 'code-buddy-agent-reasoning-execution-bundle';
  const bundleRow = appPage.getByTestId(`test-catalog-row-${bundleId}`);
  await expect(bundleRow).toBeVisible();
  await expect(bundleRow).toContainText('reasoning execution bundle');
  await expect(bundleRow).toContainText('CodeBuddyAgent');
  await expect(bundleRow).toContainText('prompt builder');
  ...
  expect(result.passed).toBeGreaterThanOrEqual(686);

  await appPage.screenshot({
    path: path.resolve(
      process.cwd(),
      '../docs/qa/code-buddy-studio/screenshots/74-test-runner-agent-reasoning-execution-bundle.png'
    ),
    fullPage: true,
  });
});
