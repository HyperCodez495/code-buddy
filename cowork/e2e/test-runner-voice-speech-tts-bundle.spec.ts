...
test('runs the Voice speech TTS bundle from the test runner window', async ({ appPage }) => {
  ...
  const bundleId = 'code-buddy-voice-speech-tts-bundle';
  const bundleRow = appPage.getByTestId(`test-catalog-row-${bundleId}`);
  await expect(bundleRow).toBeVisible();
  await expect(bundleRow).toContainText('speech TTS bundle');
  await expect(bundleRow).toContainText('wake-word fallback');
  await expect(bundleRow).toContainText('voice-to-code');
  ...
  await appPage.screenshot({
    path: path.resolve(
      process.cwd(),
      '../docs/qa/code-buddy-studio/screenshots/87-test-runner-voice-speech-tts-bundle.png'
    ),
    fullPage: true,
  });
});
