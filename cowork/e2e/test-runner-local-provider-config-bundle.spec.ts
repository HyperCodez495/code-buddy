...
test('runs the Cowork local provider config bundle from the test runner window', async ({ appPage }) => {
  ...
  const bundleId = 'code-buddy-cowork-local-provider-config-bundle';
  const bundleRow = appPage.getByTestId(`test-catalog-row-${bundleId}`);
  await expect(bundleRow).toBeVisible();
  await expect(bundleRow).toContainText('local provider config bundle');
  await expect(bundleRow).toContainText('Ollama');
  await expect(bundleRow).toContainText('LM Studio');
  ...
  await appPage.screenshot({
    path: path.resolve(
      process.cwd(),
      '../docs/qa/code-buddy-studio/screenshots/101-test-runner-local-provider-config-bundle.png'
    ),
    fullPage: true,
  });
});
