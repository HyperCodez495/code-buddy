...
test('runs the Channels messaging adapters bundle from the test runner window', async ({ appPage }) => {
  ...
  const bundleId = 'code-buddy-channels-messaging-adapters-bundle';
  const bundleRow = appPage.getByTestId(`test-catalog-row-${bundleId}`);
  await expect(bundleRow).toBeVisible();
  await expect(bundleRow).toContainText('messaging adapters bundle');
  await expect(bundleRow).toContainText('WhatsApp');
  await expect(bundleRow).toContainText('offline queue');
  ...
  await appPage.screenshot({
    path: path.resolve(
      process.cwd(),
      '../docs/qa/code-buddy-studio/screenshots/70-test-runner-channels-messaging-adapters-bundle.png'
    ),
    fullPage: true,
  });
});
