...
test('runs the browser and desktop automation bundle from the test runner window', async ({
  appPage,
}) => {
  ...
  const bundleId = 'code-buddy-browser-desktop-automation-bundle';
  const bundleRow = appPage.getByTestId(`test-catalog-row-${bundleId}`);
  await expect(bundleRow).toBeVisible();
  await expect(bundleRow).toContainText('browser desktop bundle');
  await expect(bundleRow).toContainText('route interceptor');
  await expect(bundleRow).toContainText('OCR');
  ...
  await appPage.screenshot({
    path: path.resolve(
      process.cwd(),
      '../docs/qa/code-buddy-studio/screenshots/77-test-runner-browser-desktop-automation-bundle.png'
    ),
    fullPage: true,
  });
});
