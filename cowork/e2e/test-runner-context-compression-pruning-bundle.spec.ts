...
test('runs the Context compression pruning bundle from the test runner window', async ({ appPage }) => {
  ...
  const bundleId = 'code-buddy-context-compression-pruning-bundle';
  const bundleRow = appPage.getByTestId(`test-catalog-row-${bundleId}`);
  await expect(bundleRow).toBeVisible();
  await expect(bundleRow).toContainText('compression pruning bundle');
  await expect(bundleRow).toContainText('Web search context');
  await expect(bundleRow).toContainText('compaction fallback');
  ...
  await appPage.screenshot({
    path: path.resolve(
      process.cwd(),
      '../docs/qa/code-buddy-studio/screenshots/86-test-runner-context-compression-pruning-bundle.png'
    ),
    fullPage: true,
  });
});
