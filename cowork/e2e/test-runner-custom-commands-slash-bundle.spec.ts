...
test('runs the Cowork custom commands slash bundle from the test runner window', async ({
  appPage,
}) => {
  ...
  const bundleId = 'code-buddy-cowork-custom-commands-slash-bundle';
  const bundleRow = appPage.getByTestId(`test-catalog-row-${bundleId}`);
  await expect(bundleRow).toBeVisible();
  await expect(bundleRow).toContainText('custom commands slash bundle');
  await expect(bundleRow).toContainText('autocomplete');
  await expect(bundleRow).toContainText('schedule');
  ...
  await appPage.screenshot({
    path: path.resolve(
      process.cwd(),
      '../docs/qa/code-buddy-studio/screenshots/106-test-runner-custom-commands-slash-bundle.png'
    ),
    fullPage: true,
  });
});
