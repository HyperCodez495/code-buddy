...
test('runs the Companion core behaviour bundle from the test runner window', async ({ appPage }) => {
  ...
  const bundleId = 'code-buddy-companion-core-behaviour-bundle';
  const bundleRow = appPage.getByTestId(`test-catalog-row-${bundleId}`);
  await expect(bundleRow).toBeVisible();
  await expect(bundleRow).toContainText('core behaviour bundle');
  await expect(bundleRow).toContainText('percepts');
  await expect(bundleRow).toContainText('mission');
  ...
  await expect(appPage.getByTestId(`test-catalog-result-${bundleId}`)).toHaveText('56 ok / 0 ko', {
    timeout: 190_000,
  });
  ...
  await appPage.screenshot({
    path: path.resolve(
      process.cwd(),
      '../docs/qa/code-buddy-studio/screenshots/76-test-runner-companion-core-behaviour-bundle.png'
    ),
    fullPage: true,
  });
});
