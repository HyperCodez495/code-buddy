...
test('runs the Config auth provider bundle from the test runner window', async ({ appPage }) => {
  ...
  const bundleId = 'code-buddy-config-auth-provider-bundle';
  const bundleRow = appPage.getByTestId(`test-catalog-row-${bundleId}`);
  await expect(bundleRow).toBeVisible();
  await expect(bundleRow).toContainText('auth provider bundle');
  await expect(bundleRow).toContainText('ChatGPT OAuth doctor');
  await expect(bundleRow).toContainText('model registry');
  ...
  await appPage.screenshot({
    path: path.resolve(
      process.cwd(),
      '../docs/qa/code-buddy-studio/screenshots/82-test-runner-config-auth-provider-bundle.png'
    ),
    fullPage: true,
  });
});
