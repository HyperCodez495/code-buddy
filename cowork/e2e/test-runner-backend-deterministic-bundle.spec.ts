...
test('runs the deterministic backend integration bundle from the test runner window', async ({
  appPage,
}) => {
  ...
  await expect(bundleRow).toContainText('deterministic integration bundle');
  await expect(bundleRow).toContainText('Fleet');
  await expect(bundleRow).toContainText('MCP');
  ...
  expect(result.passed).toBeGreaterThanOrEqual(55);

  await appPage.screenshot({
    path: path.resolve(
      process.cwd(),
      '../docs/qa/code-buddy-studio/screenshots/64-test-runner-backend-deterministic-bundle.png'
    ),
    fullPage: true,
  });
});
