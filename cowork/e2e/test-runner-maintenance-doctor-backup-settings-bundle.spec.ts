...
test('runs the Maintenance doctor backup settings bundle from the test runner window', async ({
  appPage,
}) => {
  ...
  const bundleId = 'code-buddy-maintenance-doctor-backup-settings-bundle';
  const bundleRow = appPage.getByTestId(`test-catalog-row-${bundleId}`);
  await expect(bundleRow).toBeVisible();
  await expect(bundleRow).toContainText('doctor backup settings bundle');
  await expect(bundleRow).toContainText('ChatGPT OAuth diagnostics');
  await expect(bundleRow).toContainText('migrations');
  ...
  await appPage.screenshot({
    path: path.resolve(
      process.cwd(),
      '../docs/qa/code-buddy-studio/screenshots/89-test-runner-maintenance-doctor-backup-settings-bundle.png'
    ),
    fullPage: true,
  });
});
