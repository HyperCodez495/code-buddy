...
test('runs the Cowork project session git bundle from the test runner window', async ({ appPage }) => {
  ...
  const bundleId = 'code-buddy-cowork-project-session-git-bundle';
  const bundleRow = appPage.getByTestId(`test-catalog-row-${bundleId}`);
  await expect(bundleRow).toBeVisible();
  await expect(bundleRow).toContainText('project session git bundle');
  await expect(bundleRow).toContainText('Git worktrees');
  await expect(bundleRow).toContainText('session CRUD');
  ...
  await appPage.screenshot({
    path: path.resolve(
      process.cwd(),
      '../docs/qa/code-buddy-studio/screenshots/97-test-runner-project-session-git-bundle.png'
    ),
    fullPage: true,
  });
});
