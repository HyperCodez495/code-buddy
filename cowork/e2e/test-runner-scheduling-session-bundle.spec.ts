...
test('runs the Cowork scheduling session bundle from the test runner window', async ({ appPage }) => {
  ...
  const bundleId = 'code-buddy-cowork-scheduling-session-bundle';
  const bundleRow = appPage.getByTestId(`test-catalog-row-${bundleId}`);
  await expect(bundleRow).toBeVisible();
  await expect(bundleRow).toContainText('scheduling session bundle');
  await expect(bundleRow).toContainText('slash /schedule');
  await expect(bundleRow).toContainText('session title');
  ...
  await appPage.screenshot({
    path: path.resolve(
      process.cwd(),
      '../docs/qa/code-buddy-studio/screenshots/100-test-runner-scheduling-session-bundle.png'
    ),
    fullPage: true,
  });
});
