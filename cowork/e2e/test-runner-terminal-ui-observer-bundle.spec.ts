...
test('runs the UI terminal observer bundle from the test runner window', async ({ appPage }) => {
  ...
  const bundleId = 'code-buddy-terminal-ui-observer-bundle';
  const bundleRow = appPage.getByTestId(`test-catalog-row-${bundleId}`);
  await expect(bundleRow).toBeVisible();
  await expect(bundleRow).toContainText('terminal observer bundle');
  await expect(bundleRow).toContainText('Ink chat interface');
  await expect(bundleRow).toContainText('screen observer');
  ...
  await appPage.screenshot({
    path: path.resolve(
      process.cwd(),
      '../docs/qa/code-buddy-studio/screenshots/81-test-runner-terminal-ui-observer-bundle.png'
    ),
    fullPage: true,
  });
});
