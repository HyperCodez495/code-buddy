...
test('runs the Gateway realtime websocket bundle from the test runner window', async ({ appPage }) => {
  ...
  const bundleId = 'code-buddy-gateway-realtime-websocket-bundle';
  const bundleRow = appPage.getByTestId(`test-catalog-row-${bundleId}`);
  await expect(bundleRow).toBeVisible();
  await expect(bundleRow).toContainText('realtime websocket bundle');
  await expect(bundleRow).toContainText('backpressure');
  await expect(bundleRow).toContainText('TLS pairing');
  ...
  expect(result.passed).toBeGreaterThanOrEqual(218);

  await appPage.screenshot({
    path: path.resolve(
      process.cwd(),
      '../docs/qa/code-buddy-studio/screenshots/68-test-runner-gateway-realtime-websocket-bundle.png'
    ),
    fullPage: true,
  });
});
