...
test('runs the Plugins skills bundle from the test runner window', async ({ appPage }) => {
  ...
  const bundleId = 'code-buddy-plugins-skills-bundle';
  const bundleRow = appPage.getByTestId(`test-catalog-row-${bundleId}`);
  await expect(bundleRow).toBeVisible();
  await expect(bundleRow).toContainText('skills bundle');
  await expect(bundleRow).toContainText('Plugin manager');
  await expect(bundleRow).toContainText('eligibility');
  ...
  await appPage.screenshot({
    path: path.resolve(
      process.cwd(),
      '../docs/qa/code-buddy-studio/screenshots/80-test-runner-plugins-skills-bundle.png'
    ),
    fullPage: true,
  });
});
