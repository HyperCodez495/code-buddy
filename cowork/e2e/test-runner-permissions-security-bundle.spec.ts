...
test('runs the permissions and security policy bundle from the test runner window', async ({
  appPage,
}) => {
  ...
  const bundleId = 'code-buddy-permissions-security-policy-bundle';
  const bundleRow = appPage.getByTestId(`test-catalog-row-${bundleId}`);
  await expect(bundleRow).toBeVisible();
  await expect(bundleRow).toContainText('security policy bundle');
  await expect(bundleRow).toContainText('Write policy');
  await expect(bundleRow).toContainText('confirmation service');
  ...
  expect(result.passed).toBeGreaterThanOrEqual(524);

  await appPage.screenshot({
    path: path.resolve(
      process.cwd(),
      '../docs/qa/code-buddy-studio/screenshots/65-test-runner-permissions-security-bundle.png'
    ),
    fullPage: true,
  });
});
