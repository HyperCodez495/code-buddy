...
test('runs the infrastructure MCP sandbox adapters bundle from the test runner window', async ({
  appPage,
}) => {
  ...
  const bundleId = 'code-buddy-infra-mcp-sandbox-adapters-bundle';
  const bundleRow = appPage.getByTestId(`test-catalog-row-${bundleId}`);
  await expect(bundleRow).toBeVisible();
  await expect(bundleRow).toContainText('MCP sandbox adapters bundle');
  await expect(bundleRow).toContainText('MCP manager');
  await expect(bundleRow).toContainText('auto-sandbox');
  ...
  await appPage.screenshot({
    path: path.resolve(
      process.cwd(),
      '../docs/qa/code-buddy-studio/screenshots/93-test-runner-infra-mcp-sandbox-adapters-bundle.png'
    ),
    fullPage: true,
  });
});
