...
test('runs the Cowork settings hooks MCP workflows bundle from the test runner window', async ({
  appPage,
}) => {
  ...
  const bundleId = 'code-buddy-cowork-settings-hooks-mcp-workflows-bundle';
  const bundleRow = appPage.getByTestId(`test-catalog-row-${bundleId}`);
  await expect(bundleRow).toBeVisible();
  await expect(bundleRow).toContainText('settings hooks MCP workflows bundle');
  await expect(bundleRow).toContainText('hooks');
  await expect(bundleRow).toContainText('workflow');
  ...
  await appPage.screenshot({
    path: path.resolve(
      process.cwd(),
      '../docs/qa/code-buddy-studio/screenshots/105-test-runner-settings-hooks-mcp-workflows-bundle.png'
    ),
    fullPage: true,
  });
});
