...
test('runs the Cowork permission path rules bundle from the test runner window', async ({
  appPage,
}) => {
  ...
  const bundleId = 'code-buddy-cowork-permission-path-rules-bundle';
  const bundleRow = appPage.getByTestId(`test-catalog-row-${bundleId}`);
  await expect(bundleRow).toBeVisible();
  await expect(bundleRow).toContainText('permission path rules bundle');
  await expect(bundleRow).toContainText('computer-use');
  await expect(bundleRow).toContainText('UNC');
  ...
  await appPage.screenshot({
    path: path.resolve(
      process.cwd(),
      '../docs/qa/code-buddy-studio/screenshots/104-test-runner-permission-path-rules-bundle.png'
    ),
    fullPage: true,
  });
});
