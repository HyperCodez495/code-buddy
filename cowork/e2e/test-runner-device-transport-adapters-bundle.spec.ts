...
test('runs the device transport adapters bundle from the test runner window', async ({ appPage }) => {
  ...
  const bundleId = 'code-buddy-device-transport-adapters-bundle';
  const bundleRow = appPage.getByTestId(`test-catalog-row-${bundleId}`);
  await expect(bundleRow).toBeVisible();
  await expect(bundleRow).toContainText('transport adapters bundle');
  await expect(bundleRow).toContainText('Device transports');
  await expect(bundleRow).toContainText('Tailscale');
  ...
  await appPage.screenshot({
    path: path.resolve(
      process.cwd(),
      '../docs/qa/code-buddy-studio/screenshots/95-test-runner-device-transport-adapters-bundle.png'
    ),
    fullPage: true,
  });
});
