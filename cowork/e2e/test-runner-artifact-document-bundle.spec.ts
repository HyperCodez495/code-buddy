...
test('runs the Cowork artifact document bundle from the test runner window', async ({ appPage }) => {
  ...
  const bundleId = 'code-buddy-cowork-artifact-document-bundle';
  const bundleRow = appPage.getByTestId(`test-catalog-row-${bundleId}`);
  await expect(bundleRow).toBeVisible();
  await expect(bundleRow).toContainText('artifact document bundle');
  await expect(bundleRow).toContainText('document workshop');
  await expect(bundleRow).toContainText('tool outputs');
  ...
  await appPage.screenshot({
    path: path.resolve(
      process.cwd(),
      '../docs/qa/code-buddy-studio/screenshots/99-test-runner-artifact-document-bundle.png'
    ),
    fullPage: true,
  });
});
