...
test('runs the Mobile supervision gateway bundle from the test runner window', async ({
  appPage,
}) => {
  ...
  const bundleId = 'code-buddy-mobile-supervision-gateway-bundle';
  const bundleRow = appPage.getByTestId(`test-catalog-row-${bundleId}`);
  await expect(bundleRow).toBeVisible();
  await expect(bundleRow).toContainText('supervision gateway bundle');
  await expect(bundleRow).toContainText('no-network acceptance plan');
  await expect(bundleRow).toContainText('approval queue');
  ...
  await appPage.screenshot({
    path: path.resolve(
      process.cwd(),
      '../docs/qa/code-buddy-studio/screenshots/90-test-runner-mobile-supervision-gateway-bundle.png'
    ),
    fullPage: true,
  });
});
