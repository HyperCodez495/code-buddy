...
test('runs the Cowork activity audit diagnostics bundle from the test runner window', async ({
  appPage,
}) => {
  ...
  const bundleId = 'code-buddy-cowork-activity-audit-diagnostics-bundle';
  const bundleRow = appPage.getByTestId(`test-catalog-row-${bundleId}`);
  await expect(bundleRow).toBeVisible();
  await expect(bundleRow).toContainText('activity audit diagnostics bundle');
  await expect(bundleRow).toContainText('global search');
  await expect(bundleRow).toContainText('renderer diagnostics');
  ...
  await appPage.screenshot({
    path: path.resolve(
      process.cwd(),
      '../docs/qa/code-buddy-studio/screenshots/102-test-runner-activity-audit-diagnostics-bundle.png'
    ),
    fullPage: true,
  });
});
