              ...
              {grouped.map((group) => (
                <details key={group.tag} open className="border border-border-subtle rounded-lg">
                  <summary className="px-3 py-2 text-xs font-medium cursor-pointer hover:bg-surface-hover flex items-center justify-between">
                    <span className="capitalize hover:text-accent hover:underline" onClick={(e) => {
                      e.stopPropagation();
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between border-b border-border pb-2">
        <h3 className="text-sm font-semibold capitalize text-accent flex items-center gap-2">
          ...
          <span className="text-[10px] px-1.5 py-0.5 rounded bg-accent/15 text-accent font-mono">weight {details.concept.weight}</span>
        </h3>
      ...
      {details.backlinks.length > 0 && (
        <div className="space-y-1">
          <span className="text-[10px] uppercase font-bold tracking-wider text-text-muted block">Related Concepts</span>
      ...
      <div className="space-y-3">
        <span className="text-[10px] uppercase font-bold tracking-wider text-text-muted block">Lessons ({details.lessons.length})</span>
        <div className="space-y-3 max-h-[48vh] overflow-y-auto pr-1">
          {details.lessons.map((lesson) => (
            <div key={lesson.id} className="p-3 bg-surface/50 border border-border-subtle rounded-lg space-y-2 text-xs">
              ...
              <div className="font-medium whitespace-pre-wrap text-text-primary">{lesson.content}</div>
              {lesson.context && (
              ...
              <div className="border-t border-border-muted pt-2 mt-2 space-y-1.5 text-[10px] text-text-muted">
                {lesson.createdBy && (
                  <div>
                    ...
                    {lesson.createdBy.note && <span className="italic">({lesson.createdBy.note})</span>}
                  </div>
                )}
                {lesson.usedBy && lesson.usedBy.length > 0 ? (
                  <div>
                    ...
                    <div className="flex flex-wrap gap-1 mt-1">
                      {lesson.usedBy.map((usage) => (
                        <span key={usage.runId} className="bg-background px-1 py-0.5 rounded border border-border-subtle font-mono text-[9px]">
                  ...
                  <div><span className="italic">Never loaded in a run yet.</span></div>
                )}
              </div>
            </div>
          ))}
