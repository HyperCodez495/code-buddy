...
export const ServerDashboard: React.FC<ServerDashboardProps> = ({ isOpen, onClose }) => {
  ...
  return (
    ...
    >
      <div
        className="w-[720px] max-w-[94vw] max-h-[85vh] bg-background border border-border rounded-xl shadow-2xl flex flex-col overflow-hidden"
      ...
      >
        ...
        {/* Stats grid */}
        {stats ? (
          <div className="grid grid-cols-4 gap-2 px-5 py-3 border-b border-border shrink-0">
          ...
          <div className="px-5 py-3 border-b border-border shrink-0 flex items-center gap-2 text-xs text-muted-foreground">
            <Activity size={11} />
        ...
        {error && (
          <div className="px-5 py-2 flex items-start gap-2 text-xs text-error bg-error/10 border-b border-border">
        ...
        <div className="flex-1 overflow-y-auto">
          {data.recent.length === 0 ? (
            <div className="text-xs text-muted-foreground text-center py-12 px-5">
              {t(
              ...
              <tbody>
                {data.recent.map((r, i) => (
                  ...
                  >
                    ...
                    <td className="px-3 py-1.5 text-secondary font-mono">{r.method}</td>
                    <td className="px-3 py-1.5 text-secondary font-mono truncate max-w-[260px]" title={r.path}>
                      {r.path}
                    </td>
                    ...
                    <td className="px-3 py-1.5 text-muted-foreground font-mono truncate max-w-[160px]" title={r.ip}>
...
const Stat: React.FC<{ label: string; value: string; tone?: 'ok' | 'warn' }> = ({
  ...
  const valueClass =
    tone === 'warn' ? 'text-warning' : tone === 'ok' ? 'text-success' : 'text-secondary';
  return (
