...
- **Kokoro Local TTS**: Implements local quantized CPU-bound voice synthesis (`src/utils/kokoro-tts.ts`) and platform-native WAV players (`src/utils/audio-player.ts`) integrated into `src/input/text-to-speech.ts` and main CLI parser (`src/index.ts` flags `--speak` and `--tts-provider kokoro`).
