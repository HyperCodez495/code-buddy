...
| OpenClaw migration | `hermes claw migrate` with 30+ categories | OpenClaw audit/imported patterns and identity files | Gap → Partial (2026-06-01) | **Update 2026-06-01:** `buddy hermes claw migrate` is now implemented against the documented OpenClaw layout (`~/.openclaw`/`~/.clawdbot`/`~/.moltbot` + `clawdbot.json`): dry-run by default, secret-safe, importing identity files/MEMORY/default model/MCP servers/skills to real consumer-backed destinations and archiving the rest for review. Fixture-tested; no real OpenClaw install validated. Full 30+ category parity remains deferred. |
