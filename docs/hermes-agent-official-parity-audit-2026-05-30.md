...
`text_to_speech`, plus exact Home Assistant `ha_*` tools.
...
| Built-in tools | Browser, file, terminal/process, web, Home Assistant, Spotify, Kanban, `execute_code`, `cronjob`, `session_search`, skills, TTS, image/video, vision, messaging, MOA, X search, Yuanbao, MCP | Code Buddy has many native tools plus Firecrawl, browser/CDP, sessions, skills, Fleet, image/vision/voice pieces, exact `kanban_*`, exact `send_message`, exact core `discord`, exact Home Assistant `ha_*`, exact `execute_code`, exact `vision_analyze`, exact `browser_vision`, and exact `text_to_speech` | Partial | Not a one-to-one tool-name or capability set; no proof for Spotify or Yuanbao. Kanban, send_message, discord core actions, Home Assistant REST actions, execute_code, vision_analyze, browser_vision, and text_to_speech now have exact prompt-tool names with native safety boundaries. |
