...
| OpenClaw Gateway bridge | `src/openclaw/gateway-bridge.ts`, `src/commands/cli/hermes-commands.ts`, `docs/fleet-guide.md` | Local compatibility adapter with CLI UX, guarded live attach and guarded response send | Reads `gateway.json`, returns secret-safe discovery, publishes an `openclaw_node_descriptor`, maps OpenClaw inbound messages to Fleet handoff drafts, prepares response previews, can attach to a daemon only after explicit approval, can send approved responses through an injected/real HTTP transport while logging redacted status, and exposes `buddy hermes claw bridge status|attach|draft|send`. Covered by `tests/openclaw/gateway-bridge.test.ts` and `tests/agent/hermes-claw-migrate-real.test.ts`. | Next: validate against a real OpenClaw daemon and add Cowork UX around the bridge. |
