...
- Deeper Cowork skill package management is mostly covered in the Fleet
  ...
  reset behavior when a hub backend is configured.
...
- Exact `skill_manage` prompt-tool parity is closed; repository tap/trust management, direct GitHub/`.well-known` discovery, and remote update diff previews are now real persisted CLI/tool surfaces, while reset behavior remains only partially proven.
