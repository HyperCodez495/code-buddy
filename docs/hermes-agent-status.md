...
| Skill creation from experience | `src/agent/research-script-skill-candidate.ts`; `buddy tools skill-candidate`; `skill_manage` prompt tool | partial — installs reviewed research-script and Learning Agent candidates; `skill_manage` now covers real list/view/create/discover, while update/patch/delete/rollback remain future review-gated lifecycle work |
