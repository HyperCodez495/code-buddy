...
const FEATURES: HermesParityFeature[] = [
  ...
  },
  {
    ...
    notes: 'Native skill coverage is good and both skill_manage candidate review plus Cowork now surface installed package state, current SKILL.md previews, candidate install-state comparisons, bounded candidate diff previews, and reviewer-gated candidate install/overwrite from the real SkillsHub lockfile, but exact Hermes hub/tap/update/reset/trust behavior is not proven identical.',
    nextWork: 'Add Cowork expanded side-by-side SKILL.md diff plus reviewer-gated installed-skill lifecycle controls.',
  },
  {
