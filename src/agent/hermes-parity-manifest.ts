...
const FEATURES: HermesParityFeature[] = [
  ...
  },
  {
    ...
    codeBuddyEvidence: ['src/codebuddy/tool-definitions/', 'src/tools/', 'src/tools/metadata.ts', 'src/channels/send-message.ts'],
    status: 'partial',
    verificationCommands: ['npx tsx src/index.ts tools profile hermes-balanced --json'],
    notes: 'Code Buddy has many native tools and now exact Kanban plus send_message tool names, but not all official third-party integrations or execute_code RPC semantics.',
