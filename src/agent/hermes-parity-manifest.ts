...
const FEATURES: HermesParityFeature[] = [
  ...
  },
  {
    ...
    codeBuddyEvidence: ['src/codebuddy/tool-definitions/', 'src/tools/', 'src/tools/metadata.ts', 'src/channels/send-message.ts', 'src/tools/execute-code-runner.ts'],
    status: 'partial',
    verificationCommands: [
      'npx tsx src/index.ts tools profile hermes-balanced --json',
      'npm test -- tests/tools/execute-code-real.test.ts tests/tools/send-message-real.test.ts tests/tools/kanban-real.test.ts --run',
    ],
    notes: 'Code Buddy has many native tools and now exact Kanban, send_message, and execute_code tool names, but not all official third-party integrations.',
