...
const FEATURES: HermesParityFeature[] = [
  ...
  },
  {
    ...
    notes: 'Comparable direction with stricter review gates. Accepted user-model observations are injected per turn and counted by prompt-size diagnostics; reusable skill outcomes now keep scored recommendation history with reasons and next actions, and approved skill_manage mutations record rollback snapshot ids in Learning Agent telemetry. `buddy hermes learning status --json` now summarizes real local runs, trajectory event counts, retrospectives, lesson candidates, user-model observations, skill scoring, pattern library state, and review gates without printing private model content, and it chooses the next retrospective from the densest unfinished trajectory. Honcho-style LLM inference and credential-free deterministic local inference both propose pending observations only; a real CLI test proves session analysis writes candidates without silently accepting them.',
