...
export const UNDERSTAND_VIDEO_TOOL: CodeBuddyTool = {
  ...
  function: {
    ...
    description: "Understand a video, local-first and $0. By default produces a timestamped transcript of what is SAID (YouTube captions → yt-dlp audio + local Whisper → local file) plus a compact research-intake card spanning the complete transcript (technology passages, claims to verify, detected links). Set visual:true to ALSO analyze what is SHOWN on screen (samples keyframes, dedups near-identical ones, describes each with a local vision model, fuses one keyframe per transcript segment into {said, shown} tuples) — ideal for code screencasts; set ocr:true to also read on-screen code/text with OCR. Set cloud:true (OPT-IN) to also send the video/URL to Gemini for a joint audio+visual timestamped answer — this sends data to Google (public/non-sensitive videos only) and degrades to the local transcript if unavailable. Returns the structured transcript and persists both artifacts to .codebuddy/video/.",
    parameters: {
      type: "object",
      properties: {
        source: {
