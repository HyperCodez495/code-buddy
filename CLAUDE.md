...
| `FINNHUB_API_KEY` | Optional, most-reliable source for the `stock_quote` tool (`src/tools/stock-quote.ts`). When set, tried FIRST; otherwise the tool falls back to free/no-key Yahoo Finance → Stooq ($0 default). `stock_quote` emits a `data:{type:'stock'\|'market',…}` payload that renders the curated stock widget inline. Base URLs overridable via `CODEBUDDY_YAHOO_FINANCE_BASE` / `CODEBUDDY_STOOQ_BASE` / `CODEBUDDY_FINNHUB_BASE` (loopback tests) |
