...
- **Safety invariants:** never edits `src/`; sandboxed scoring only; held-out hidden from the proposer; `authored__` namespace can't shadow built-ins; append-only archive; opt-in default-off kill-switch. **Skills path** (authoring skills via `skill_manage` under the same held-out gate) is the remaining unwired piece.
